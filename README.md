...
На Railway бот специально не запустится, если:

- `DB_PATH` не начинается с `/data/`;
- папка `/data` не существует, то есть Volume не подключён.

Так лучше: бот покажет ошибку настройки, а не создаст новую пустую базу.

Перед изменениями графика бот автоматически создаёт резервные копии базы в `db_backups/`.
